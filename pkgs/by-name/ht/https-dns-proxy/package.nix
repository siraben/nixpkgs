...
stdenv.mkDerivation (finalAttrs: {
  ...
  postInstall = ''
    install -Dm444 -t $out/share/doc/https-dns-proxy ../{LICENSE,*.md}
    install -Dm444 -t $out/share/https-dns-proxy/munin ../munin/*
    # the systemd service definition is garbage, and we use our own with NixOS
    mv $out/lib/systemd $out/share/https-dns-proxy
    rmdir $out/lib
  '';
