...
stdenv.mkDerivation (finalAttrs: {
  ...
  passthru.updateScript = writeScript "update.sh" ''
    ...
    oldVersion="$(nix-instantiate --eval -E "with import ./. {}; lib.getVersion nanorc" | tr -d '"' | sed 's|\\.|-|g')"
    ...
    if [ "$oldVersion" != "$latestTag" ]; then
      ...
      update-source-version nanorc "$newTag" --version-key=version --print-changes
    else
      echo "nanorc is already up-to-date"
    fi
  '';
