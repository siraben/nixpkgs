...
# adding a plugin for another printer shouldn't be too difficult, but you need the firmware to test...
{
  v330 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "Plugin to support " + finalAttrs.passthru.hw + " scanner in sane";
    };
  });
  ...
  v600 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan esci x820 plugin for " + finalAttrs.passthru.hw;
    };
  });
  x770 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan esci x770 plugin for " + finalAttrs.passthru.hw;
    };
  });
  f720 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan esci f720 plugin for " + finalAttrs.passthru.hw;
    };
  });
  s80 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan esci s80 plugin for " + finalAttrs.passthru.hw;
    };
  });
  s600 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan gt-s600 plugin for " + finalAttrs.passthru.hw;
    };
  });
  s650 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan GT-S650 for " + finalAttrs.passthru.hw;
    };
  });
  x750 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan GT-X750 for " + finalAttrs.passthru.hw;
    };
  });
  gt1500 = stdenv.mkDerivation (finalAttrs: {
    ...
    meta = common_meta // {
      description = "iscan GT-1500 for " + finalAttrs.passthru.hw;
    };
  });
