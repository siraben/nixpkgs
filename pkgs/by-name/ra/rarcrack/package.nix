{
  lib,
  stdenv,
  fetchFromGitHub,
  libxml2,
  file,
  p7zip,
  unrar,
  unzip,
}:

stdenv.mkDerivation {
  pname = "rarcrack";
  version = "0.2";

  src = fetchFromGitHub {
    owner = "jaredsburrows";
    repo = "Rarcrack";
    rev = "35ead64cd2b967eec3e3e3a4c328b89b11ff32a0";
    sha256 = "134fq84896w5vp8vg4qg0ybpb466njibigyd7bqqm1xydr07qrgn";
  };

  nativeBuildInputs = [ unzip ];
  buildInputs = [
    libxml2
    file
    p7zip
    unrar
  ];
  buildFlags = lib.optional stdenv.cc.isClang "CC=clang";
  installFlags = [ "PREFIX=\${out}" ];

  patchPhase = ''
    substituteInPlace rarcrack.c --replace "file -i" "${file}/bin/file -i"
  '';

  env.NIX_CFLAGS_COMPILE = "-Wno-error=incompatible-pointer-types";

  preInstall = ''
    mkdir -p $out/bin
  '';

  meta = {
    description = "This program can crack zip, 7z and rar file passwords";
    longDescription = ''
      If you forget your password for compressed archives (rar, 7z, zip), this program is the solution.
      This program uses a brute-force algorithm to find the correct password. You can specify which characters will be used in password generation.
      Warning: Please don't use this program for anything illegal!
    '';
    homepage = "https://github.com/jaredsburrows/Rarcrack";
    license = lib.licenses.gpl2Only;
    maintainers = with lib.maintainers; [ davidak ];
    platforms = with lib.platforms; unix;
  };
}
