...
* `extraNativeBuildInputs` (optional, deprecated), an additional list of derivations to add to `nativeBuildInputs`,
* `overrideNativeBuildInputs` (optional) replaces the default list of derivations to which `nativeBuildInputs` and `extraNativeBuildInputs` add extra elements,
...
* `extraBuildInputs` (optional, deprecated), an additional list of derivations to add to `buildInputs`,
* `overrideBuildInputs` (optional) replaces the default list of derivations to which `buildInputs` and `extraBuildInputs` add extra elements,
* `propagatedBuildInputs` (optional) is passed as is to `mkDerivation`; we recommend using this for Rocq libraries and Rocq plugin dependencies, as this makes sure the paths of the compiled libraries and plugins will always be added to the build environments of subsequent derivations, which is necessary for Rocq packages to work correctly,
...
* `dropAttrs`, `keepAttrs`, `dropDerivationAttrs` are all optional and allow tuning which attribute is added or removed from the final call to `mkDerivation`.
