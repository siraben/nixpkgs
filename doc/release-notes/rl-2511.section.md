...
- `haskellPackages` and the package sets under `haskell.packages` no longer expose an `llvmPackages` attribute,
  ...
  Haskell packages usually only need to depend on an LLVM version matching GHC if they force the use of the LLVM
...
- `installShellFiles` now allows `installManPage` to take a piped input and adds the `--name` flag for renaming the file when installed. You can also append `--` to opt-out of all subsequent parsing.

- `kbd` package's `outputs` now include `man` and `scripts` outputs. The `unicode_start` and `unicode_stop` Bash scripts are now part of the `scripts` output, allowing most usages of the `kbd` package to not pull in `bash`.
...
- `odoo16` has been dropped due to end of support by upstream, consider upgrading to a supported version.
...
- The `telegram-desktop` package now uses `Telegram` for its binary. The previous name was `telegram-desktop`. This is due to [an upstream decision](https://github.com/telegramdesktop/tdesktop/commit/56ff5808a3d766f892bc3c3305afb106b629ef6f) to make the name consistent with other platforms.
...
- HTTP3 support has been enabled in `nginx`, `openresty`, `angie` and `tengine`. The `nginxQuic` and `angieQuic` package flavors have been removed.
...
- `prl-tools` has been moved out of `linuxPackages` because Parallels Guest Tools have been driverless since 26.1.0.
...
- `simplesamlphp` has been removed since the package was severely outdated, unmaintained in Nixpkgs and had known vulnerabilities.

- The `clickhouse` package now tracks the stable upstream version per [upstream's
...
- `types.either` silently accepted mismatching types when used in `freeformType`. Module maintainers should fix the used type.
  ...
  types.either was used to bootstrap other types. This also affects the following types:
  - `oneOf`
  - `number`
  - `numbers.*`
