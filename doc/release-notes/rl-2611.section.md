...
- Python package runtime dependencies are no longer propagated as build inputs.
  `buildPythonPackage` records `dependencies` as a cycle-safe runtime graph,
  while `buildPythonApplication`, `toPythonApplication`, and
  `python.withPackages` compose that graph when it is needed. Python modules
  formerly listed in `propagatedBuildInputs` are migrated automatically;
  non-Python inputs retain their propagation behavior. Non-Python consumers
  that need to import a module and its dependencies should use
  `python.pkgs.requiredPythonModules`.

  Checks for standard Python build formats now run in
  `passthru.tests.python`, independently of the installed package. Runtime,
  import, and conflict validation is included there and the tests are exposed
  by the Python Hydra release job. Exceptional packages can use
  `separateChecks = false`.

  Dependencies imported while building a wheel must now also be declared in
  `build-system`; putting them only in `dependencies` no longer makes them
  accidental build inputs.
