...
- The Factor programming language has been updated to version 0.101, bringing various improvements to UI rendering and HiDPI support as well as support for Unicode 17.0.0.
  ...
  It has been part of the MELPA package set for a while already and must be taken from there.
...
- The `duckstation` package has been removed, as it was requested by upstream and build sources were changed to be incompatible with NixOS.
...
- `geph` package's built-in GUI `geph5-client-gui` has been [removed](https://github.com/geph-official/geph5/commit/f2221fb8386312daf2cef05483ebb353ff48bdb4) by the upstream. All users who wish to continue using the GUI should install `gephgui-wry`, which is consistent with the official release version.
...
- The `lunarvim` package has been removed, as it was abandoned upstream and relied on an old version of `neovim` to work properly.
...
- The `command-not-found` package will be enabled by default if the source of nixpkgs contains the file `programs.sqlite`. This is the case if a nixpkgs tarball from <https://channels.nixos.org> is used. This usage will also make the database of `command-not-found` stateless.
...
- `python3Packages.pygame` has been renamed to `python3Packages.pygame-original`; from Python 3.14, the attribute `python3Packages.pygame` will default to the more actively maintained `python3Packages.pygame-ce`.
...
- `pocket-id` has been updated to version 2, which contains [breaking changes](https://pocket-id.org/docs/setup/major-releases/migrate-v2).
...
- `asio` (standalone version of `boost::asio`) has been updated from 1.24.0 to 1.36.0. Some breaking changes were introduced between these
  ...
  `asio` also no longer propagates `boost`, as it is used independently of `boost` in most cases.
...
- `opensmtpd-filter-dkimsign` is now installed into `libexec/smtpd` instead of `libexec/opensmtpd` so that now it is properly linked into the environment built by `services.opensmtpd.procPackages`. If you hardcoded the path to `filter-dkimsign`, please consider using this option.
...
- `hyphen` now supports over 40 language variants through `hyphenDicts` and now allows enabling all supported languages through `hyphenDicts.all`.
...
- `canokey-qemu` support for `qemu` was restored (although disabled by default), after being marked as broken since nixpkgs 25.11. Please note that the format of canokey files has changed, and that some data created with older canokey-qemu releases cannot be read by the current version. See the [documentation](https://github.com/canokeys/canokey-qemu/tree/v1?tab=readme-ov-file#compatibility-warning) for details.
