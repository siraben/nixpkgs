...
- MATE has been updated to 1.28.
  ...
  - To use the Wayland session, enable `services.xserver.desktopManager.mate.enableWaylandSession`. This is opt-in for now as it is at an early stage and introduces a new set of Wayfire closures. Due to [known issues with LightDM](https://github.com/canonical/lightdm/issues/63), we suggest using SDDM as the display manager.
...
- `boot.supportedFilesystems` and `boot.initrd.supportedFilesystems` are now attribute sets instead of lists. Assignment from lists as done previously is still supported, but checking whether a filesystem is enabled must now be done using `supportedFilesystems.fs or false` instead of using `lib.elem "fs" supportedFilesystems` as was done previously.
...
- switch-to-configuration does not directly call systemd-tmpfiles anymore.
  ...
  allows restarting multiple services that are ordered before sysinit.target
...
- `bacula` now allows configuring `TLS` for encrypted communication.
...
- A new abstraction to create both read-only and writable overlay file
