...
- Every NixOS configuration now provides `system.build.nspawn`, a launcher for
  running an nspawn-specific variant of the configuration in
  `systemd-nspawn`. Container-only settings can be specified with
  [`virtualisation.nspawnVariant`](#opt-virtualisation.nspawnVariant).
