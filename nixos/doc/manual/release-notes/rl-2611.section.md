...
- The default package used by `services.stirling-pdf` now excludes login and
  other additional features, which are proprietary. Deployments that use these
  features must set `services.stirling-pdf.package = pkgs.stirling-pdf-unfree`
  and allow the `stirling-pdf-unfree` unfree package. The
  `stirling-pdf-desktop` package is also now correctly marked as unfree.
