...
- The `login` PAM service now creates a private kernel session keyring and links
  the user's keyring into it. This prevents session-local keys from leaking
  between concurrent logins and makes user keys available to commands such as
  encrypted Bcachefs mounts. The session keyring is revoked on logout, making
  keys reachable only through it inaccessible; UID-wide user keys retain their
  existing lifetime.
