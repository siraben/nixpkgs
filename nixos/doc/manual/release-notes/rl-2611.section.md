...
- `services.qbittorrent`: when `serverConfig` is non-empty, the module now
  ...
  queueing silently. Users of `serverConfig` that have not set
  `QueueingSystemEnabled` must set
  `serverConfig.BitTorrent.Session.QueueingSystemEnabled = true` to retain the
  historical default after upgrading. Users that do not use `serverConfig` or
  have already set `QueueingSystemEnabled` are not impacted.
