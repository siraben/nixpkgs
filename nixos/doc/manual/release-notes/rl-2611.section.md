...
- Plymouth now uses the simpledrm firmware framebuffer immediately by default.
  This avoids delayed or text-mode prompts when hardware-specific display
  drivers are unavailable during early boot. Systems whose firmware selects an
  unsuitable display or resolution can restore the previous behavior with
  [`boot.plymouth.useSimpledrm = false`](#opt-boot.plymouth.useSimpledrm).
