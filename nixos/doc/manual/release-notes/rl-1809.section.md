...
- When enabled the `iproute2` will copy the files expected by ip route (e.g., `rt_tables`) in `/etc/iproute2`. This allows writing aliases for routing tables, for instance.
...
- The `programs.screen` module allows configuring `/etc/screenrc`; however, the module behaved fairly counterintuitively, as the config existed but the package wasn't available. Since 18.09, `pkgs.screen` will be added to `environment.systemPackages`.
