...
- There is now a set of `confinement` options for `systemd.services`, which allows restricting services to a chrooted environment that only contains the store paths from the runtime closure of the service.
...
- The option `users.ldap.bind.password` was renamed to `users.ldap.bind.passwordFile`, and needs to be readable by the `nslcd` user. The same applies to the new `users.ldap.daemon.rootpwmodpwFile` option.
