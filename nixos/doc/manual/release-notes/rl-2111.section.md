...
- [ucarp](https://download.pureftpd.org/pub/ucarp/README), a userspace implementation of the Common Address Redundancy Protocol (CARP). Available as [networking.ucarp](options.html#opt-networking.ucarp.enable).
...
- [Hockeypuck](https://github.com/hockeypuck/hockeypuck), an OpenPGP Key Server. Available as [services.hockeypuck](#opt-services.hockeypuck.enable).
...
- [iio-niri](https://github.com/Zhaith-Izaliel/iio-niri), a utility that listens to `iio-sensor-proxy` and updates Niri output orientation depending on the accelerometer orientation.
...
- The wordpress module provides a new interface which allows the use of different webservers with the new option [`services.wordpress.webserver`](options.html#opt-services.wordpress.webserver).  Currently `httpd`, `caddy` and `nginx` are supported. The definitions of wordpress sites should now be set in [`services.wordpress.sites`](options.html#opt-services.wordpress.sites).

  Sites definitions that use the old interface are automatically migrated in the new option. This backward compatibility will be removed in 22.05.

- The dokuwiki module provides a new interface which allows the use of different webservers with the new option [`services.dokuwiki.webserver`](options.html#opt-services.dokuwiki.webserver).  Currently `caddy` and `nginx` are supported. The definitions of dokuwiki sites should now be set in [`services.dokuwiki.sites`](options.html#opt-services.dokuwiki.sites).
...
- ipfs now defaults to not listening on your local network. This setting was changed because server providers won't accept port scanning on their private network. If you have several ipfs instances running on a network you own, feel free to change the setting `ipfs.localDiscovery = true;`. localDiscovery enables different instances to discover each other and share data.
