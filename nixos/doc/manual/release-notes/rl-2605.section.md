...
- [rqbit](https://github.com/ikatson/rqbit), a bittorrent client written in Rust. It has an HTTP API and a Web UI, and can be used as a library. Available as [services.rqbit](#opt-services.rqbit.enable).
...
- []{#sec-release-26.05-incompatibilities-profiles-hardened-removed} `profiles/hardened` has been removed, because:
   ...
   - See the [NixOS Hardening](https://wiki.nixos.org/wiki/NixOS_Hardening) wiki page for hardening options.
...
- `services.stalwart-mail` has been renamed to [`services.stalwart`](#opt-services.stalwart.enable) to align with upstream re-brand as an e-mail and collaboration server. Other notable breaking changes to the module:

  - Addition of a module-specific `stateVersion` option, which on existing installations of Stalwart must be set to the same as `system.stateVersion`.
  ...
  - `systemd.services.stalwart` owned by `stalwart:stalwart`. The `user` and `group` are configurable via `services.stalwart.user` and `services.stalwart.group`, respectively. By default, if `stateVersion` is older than `26.05`, will fall back to the legacy value of `stalwart-mail` for both `user` and `group`.
  - Default value for `services.stalwart.dataDir` has changed to `/var/lib/stalwart`. If `stateVersion` is older than `26.05`, will fall back to the legacy value of `/var/lib/stalwart-mail`.
  - Default tracer name and type have changed to `journal`. If `stateVersion` is older than `26.05`, will fall back to the legacy value of `stdout`.
...
- `walker` has been updated to 2.0.0+, which is a complete rewrite in rust.
  ...
  The way keybinds and actions are handled has been completely revamped. Please refer to the [default config](https://raw.githubusercontent.com/abenz1267/walker/refs/heads/master/resources/config.toml).
...
- The [networking.wireless](#opt-networking.wireless.enable) module has been security hardened by default: the `wpa_supplicant` daemon now runs under an unprivileged user with restricted access to the system.
  ...
  Similarly, the `ctrl_interface` directory set in `wpa_supplicant`'s conf must be writable by the `wpa_supplicant` user so that the `wpa_supplicant` daemon can start successfully. If you were changing `ctrl_interface` in [extraConfig](#opt-networking.wireless.extraConfig) or in `/etc/wpa_supplicant/imperative.conf`, please remove that line.
...
- `security.acme` now defaults to a dynamic renewal duration if
  ...
  remains unset. This accommodates certificates with different ACME profile:
    ...
    - For short-lived certificates with a total validity below 10 days, renewal
...
- [services.resolved](#opt-services.resolved.enable) module was converted to RFC42-style settings. The moved options have also been renamed to match the upstream names. Aliases mean current configs will continue to function, but users should move to the new options when convenient.
...
- InvoicePlane with the Caddy webserver (`services.invoiceplane.webserver = "caddy"`) now sets up sites with Caddy's automatic HTTPS instead of HTTP-only.
  ...
  If you set custom Caddy options for an InvoicePlane site, migrate these options by removing `http://` from `services.caddy.virtualHosts."http://example.com"`.
