...
- [mediamtx](https://github.com/aler9/mediamtx), ready-to-use RTSP / RTMP / HLS server and proxy that allows reading, publishing and proxying video and audio streams. Available as [services.mediamtx](#opt-services.mediamtx.enable).
...
- [usbrelayd](https://github.com/darrylb123/usbrelay), a USB Relay MQTT daemon. Available as [services.usbrelayd](#opt-services.usbrelayd.enable).
...
- For new installations `virtualisation.oci-containers.backend` is now set to `podman` by default.
  If you still want to use Docker on systems where `system.stateVersion` is set to `"22.05"`, set `virtualisation.oci-containers.backend = "docker";`. Old systems with older `stateVersion`s stay with "docker".
...
- `git` no longer hardcodes the path to OpenSSH's SSH binary to reduce the number of rebuilds. If you are using git with SSH remotes and do not have an SSH binary in your environment, consider adding `openssh` to it or switching to `gitFull`.
...
- The Keycloak package (`pkgs.keycloak`) has been switched from the
  ...
  - `services.keycloak.extraConfig` has been removed in favor of the
    ...
    one, this unfortunately means that there's no straightforward way
...
- Services in the `hadoop` module previously set `openFirewall` to true by default.
  ...
  `openFirewall = true;` to be added to Hadoop services when upgrading from NixOS 21.11.
...
- `switch-to-configuration` (the script that is run when running `nixos-rebuild switch` for example) has been reworked
    ...
    * The script now uses a proper ini-file parser to parse systemd units. Some values are now only searched in one section instead of in the entire unit. This is only relevant for units that don't use the NixOS systemd module.
