...
- Software that uses the `crypt` password hashing API is now using the implementation provided by [`libxcrypt`](https://github.com/besser82/libxcrypt) instead of glibc's, which enables support for more secure algorithms.
  ...
  - This includes system login passwords. Given this, we **strongly encourage** all users to update their system passwords, as you will be unable to log in if password hashes are not migrated by the time their support is removed.
...
- `haskellPackages.callHackage` is not always invalidated if `all-cabal-hashes` changes, leading to fewer rebuilds of Haskell dependencies.
