...
- The `waagent` service now performs provisioning.
...
- GraalPy (`graalCEPackages.graalpy`), TruffleRuby
  ...
  GraalNodeJS (`grallCEPackages.graalnodejs`) are now independent of the main
  GraalVM derivation.
...
- `baloo`, the file indexer and search engine used by KDE now has a patch to
  prevent files from constantly being reindexed when the device IDs of their
  underlying storage change. This happens frequently when using btrfs or
...
- `nixos-rebuild {switch,boot,test,dry-activate}` runs the system
  ...
  switch in an isolated environment, which could possibly break post-switch
  scripts that depend on things like environment variables being set. If you
  want to opt out of this behavior for now, you may set the
...
- `nano` was removed from `environment.defaultPackages`. To not leave systems
  without an editor, `programs.nano.enable` is now enabled by default.
...
- The `extend` function of `llvmPackages` has been removed due to it coming from
...
- Node.js v14 and v16 have been removed because they reached end of life. Any dependent packages that contributors were not able to reasonably upgrade were also removed after their maintainers received a month's notice.
...
- The `services.sslh` module has been updated to follow [RFC
  ...
  [services.sslh.settings](#opt-services.sslh.settings), which allows changing
  any of the settings in {manpage}`sslh(8)`.

  In addition, the newly added option
  [services.sslh.method](#opt-services.sslh.method) allows switching between
...
- [LibreNMS](https://www.librenms.org), an auto-discovering PHP/MySQL/SNMP-based
...
- [virt-manager](https://virt-manager.org/), a UI for managing virtual
