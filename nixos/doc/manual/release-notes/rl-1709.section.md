...
- **In a QEMU-based virtualization environment, the network interface names changed, for example, from `enp0s3` to `ens3`.**
...
- The `ssh-agent` user service is not started by default anymore. Use `programs.ssh.startAgent` to enable it if needed. There is also a new `programs.gnupg.agent` module that creates a `gpg-agent` user service. It can also serve as an SSH agent if `enableSSHSupport` is set.
