...
- [postfix-tlspol](https://github.com/Zuplu/postfix-tlspol), an MTA-STS and DANE resolver and TLS policy server for Postfix. Available as [services.postfix-tlspol](#opt-services.postfix-tlspol.enable).
...
- [Spoolman](https://github.com/Donkie/Spoolman), an inventory management system for Filament spools. Available as [services.spoolman](#opt-services.spoolman.enable).
...
- mate-wayland-session 1.28.4 is now using the default wayfire decorator instead of firedecor, thus `services.xserver.desktopManager.mate.enableWaylandSession` is no longer shipping firedecor. If you are experiencing broken window decorations after upgrade, back up and remove `~/.config/mate/wayfire.ini` and log in again.
...
- `services.pfix-srsd` now automatically integrates with the local Postfix instance, when enabled. This behavior can be disabled using the `services.pfix-srsd.configurePostfix` option.

- `services.postsrsd` now automatically integrates with the local Postfix instance, when enabled. This behavior can be disabled using the [services.postsrsd.configurePostfix](#opt-services.postsrsd.configurePostfix) option.
