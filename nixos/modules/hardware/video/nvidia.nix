      ...
      lib.mkMerge [
        # Common
        {
          ...
          boot = {
            ...
            # The proprietary driver can load UVM after the base module with a soft dependency.
            # Loading UVM eagerly is unsafe when the GPU becomes available late, as on some cloud VMs.
            extraModprobeConfig = lib.optionalString (!useOpenModules) ''
          ...
          services.udev.extraRules =
            # With the open driver, loading UVM eagerly or as a soft dependency can race the GPU probe.
            # A bind event is emitted only after the base driver has successfully probed the device.
            lib.optionalString useOpenModules ''
              ACTION=="add|bind", SUBSYSTEM=="pci", DRIVER=="nvidia", RUN+="${lib.getExe' pkgs.kmod "modprobe"} nvidia_uvm"
            ''
            + ''
              # Create NVIDIA device nodes when their modules are loaded.
