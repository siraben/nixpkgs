      ...
      lib.mkMerge [
        # Common
        {
          ...
          services.udev.extraRules = ''
            # Create NVIDIA device nodes when the modules are loaded.
            ...
            # A GPU can bind after the module event, for example when hot-plugged.
            ACTION=="bind", SUBSYSTEM=="pci", DRIVER=="nvidia", RUN+="${pkgs.runtimeShell} -c 'for i in $$(cat /proc/driver/nvidia/gpus/*/information | grep Minor | cut -d \  -f 4); do [ -e /dev/nvidia$${i} ] || mknod -m 666 /dev/nvidia$${i} c 195 $${i}; done'"
          '';
          hardware.graphics =
            let
              icd = [
